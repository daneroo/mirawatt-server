...
var processRawDay = function(json,T,startStr){
    ...
    var verbose=false;
    var canonical = rawToCanonical(data,startStr,T,verbose);
...
function fetch(table,dayStr,cb){
    ...
    http.get(options, function(res) {
        ...
        res.addListener('end', function() {
            var startStr = dayStr+'T00:00:00Z';
            console.log('raw::',startStr,responseBody.length);
            var data = JSON.parse(responseBody);

            var verbose=false;
            var T=1;
            var values = rawToCanonical(data,startStr,T,verbose);
            var signal = {
                stamp : startStr,
                T : T,
                // Q: 10,
            ...
            console.log('can::',startStr,json.length);
            gzipMax(json, function(error,gzbuf){
                if (error) {
